...
-- | Create a trivial nix derivation that runs the fetcher and extracts the source. This is used in 'NvFetcher.ExtractSrc' to extract files from the source.
fetcherToExtractDrv :: NixFetcher Fetched -> Text -> FilePath -> NixExpr
fetcherToExtractDrv (toNixExpr -> fetcherExpr) (appName) (fileExpr) =
  [trimming|
    with import <nixpkgs> { };
    stdenv.mkDerivation {
      name = "extract-app-name";
      src = $fetcherExpr;
      nativeBuildInputs = [ $appNameExpr cacert  unzip ];
      dontBuild = true;
      installPhase = ''
        echo $file
        $appNameQuoted $file > $$out
      '';
    }
  |]
 where
  file = quote . T.pack $ fileExpr
  appNameQuoted = quoteIfNeeds appName
  appNameExpr = toNixExpr appName
...
instance ToNixExpr PackageResult where
  toNixExpr PackageResult{..} =
   ...
   where
      ...
      if T.null extract
        then ""
          ...
          [trimming|
            ...
            |]
    prefetched = maybe "" (\pf -> "prefetch = {\n" <> pf' pf <> "\n};") _prprefetchFiles
     where
      pf' pf = T.unlines [quoteIfNeeds (T.pack src) <> " = " <> quote v <> ";" | (src, Just v) <- HMap.toList pf]
    cargo = fromMaybe "" $ do
    ...
    joined = extracted' <> cargo <> passthru <> date <> prefetched
